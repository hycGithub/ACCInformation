 ...
 *SIM工具包应用程序级服务。 与来自STK UI元素的Telephopny消息，应用程序启动和用户输入进行交互。
 */
public class StkAppService extends Service implements Runnable {
    ...
    // 静态类型的实例变量，在onCreate中初始化
    static StkAppService sInstance = null;

    //mStkService为 CatService 类型数组，
    ...
    public void onCreate() {
        ...
        //初始化mStkContext、mStkService 两者都是数组类型 mStkService 是CatService，mStkContext是StkContext
    ...
    /**
     *
     * @param intent 包含op 和simId德值
     * @param startId
     *
     * fun：一些初始化的操作，主要是根据op和simId 创建一个msg，然后发送给ServiceHandler的实例处理
     */
    @Override
    ...
    public void run() {
        //在线程中创建了一个ServiceHandlerHandler实例
        Looper.prepare();
     ...
     * StkMenuActivity使用的包api指示它是否在前台。
     */
     ...
     * StkDialogActivity使用的包api指示它是否在前台。
     */
    ...
    /**
     * ServiceHandler 为一个Handler ，sendMessage（msg）用来 往looper中发送消息，handleMessage（msg）用来接收一个msg
     * 并根据msg内容 进行处理。
     */
    private final class ServiceHandler extends Handler {
        @Override
        public void handleMessage(Message msg) {
            //传入的msg为CatCmdMessage类型
            if(null == msg) {
            ...
            // 一个simId对应着一个 mStkContext 上下文实例
            ...
            case OP_LAUNCH_APP:
                ...
                //如果插槽ID有挂起的活动，只需完成它并创建一个新的处理挂起的命令。
            ...
            case OP_CMD:
                ...
                // 1. Interactive - user's response is required.交互式 - 用户的响应是必需的。
                // 2. Informative - display a message, no interaction with the user.信息性 - 显示消息，不与用户交互。
                //
                ...
                //交互式命令不能互相覆盖。 因此，如果命令已在进行中，我们需要将下一个命令排队，直到用户响应或超时到期。
                // 依据cmdType 类型判断
    ...
    private void handleCmdResponse(Bundle args, int slotId) {
        ...
        case RES_ID_TIMEOUT:
            ...
            //GCF测试用例27.22.4.1.1预期序列1.5（显示文本，延迟后清除消息，成功）期望结果代码OK。
            // 如果命令限定符指定不需要用户响应，则发送OK而不是NO_RESPONSE_FROM_USER
    ...
    private void launchMenuActivity(Menu menu, int slotId) {
        ...
        if (menu == null) {
            ...
            //如果最后一个待处理菜单是二级菜单，则“STATE”应为“STATE_SECONDARY”。
            ...
            //我们不知道，我们将让getFlagActivityNoUserAction决定。
